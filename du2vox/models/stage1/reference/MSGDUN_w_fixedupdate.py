...
# 改进 ①：尺度自适应 Cross Attention（替换 concat+MLP）
...
# 改进 ②：Physics-weighted Global KV（基于系统矩阵 M）
...
class FixedThreshold(nn.Module):
    """
    Global learnable soft-threshold (ISTA-style)
    """
    def __init__(self, init_thresh=0.01):
        super().__init__()
        self.thresh = nn.Parameter(torch.tensor(init_thresh))

    def forward(self, u):
        return torch.sign(u) * torch.relu(torch.abs(u) - self.thresh)


# 改进 ④：跨迭代特征记忆（Feature Memory）
...
class FixedUpdateBlock(nn.Module):
    """
    Ablation: Fixed sparsity (no adaptive, no node-wise)
    """
    def __init__(self, init_thresh=0.01):
        super().__init__()
        self.fixed_thresh = FixedThreshold(init_thresh)

    def forward(self, x, grad, feat=None):
        u = x - grad
        return self.fixed_thresh(u)
...
class BasicBlock_with_cross_attention(nn.Module):
    def __init__(self, L, A, L0, L1, L2, L3, knn_idx, feat_dim=6):
        ...
        self.attention = MultiScaleKNNGraphAttention(
                                feat_dim=feat_dim,
                                knn_idx=knn_idx,   # 预计算好的
                                M=A                # 系统矩阵（可选）
                            )
        self.update = FixedUpdateBlock(init_thresh=0.01)
    ...
    def forward(self, x, B):
        ...
        feat_attn  = self.attention(x3, x0, x1, x2)
        grad = self.grad_proj(feat_attn) 
